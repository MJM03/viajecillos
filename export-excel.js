(() => {
  ...
  async function buildWorkbook(){
    ...
    trip.forEach((day,idx)=>{
      ...
      ws.getCell(r,32).numFmt='dd/mm/yyyy hh:mm';
    });
  ...
  async function openExcel(){
    ...
    if(tab){tab.document.write('<title>Viajecillos — Excel</title><meta name="viewport" content="width=device-width,initial-scale=1"><body style="font-family:-apple-system,BlinkMacSystemFont,Segoe UI,sans-serif;padding:32px;color:#111827"><h2>Generando Excel…</h2><p>En unos segundos se abrirá el archivo.</p></body>');tab.document.close()}
    try{
      const wb=await buildWorkbook(),buffer=await wb.xlsx.writeBuffer(),blob=new Blob([buffer],{type:'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet'}),url=URL.createObjectURL(blob);
      if(tab){tab.location.href=url;setTimeout(()=>URL.revokeObjectURL(url),60000)}else{const a=document.createElement('a');a.href=url;a.download=`Viajecillos_Lider_2_${new Date().toISOString().slice(0,10)}.xlsx`;document.body.appendChild(a);a.click();a.remove();setTimeout(()=>URL.revokeObjectURL(url),5000)}
    }catch(err){console.error(err);if(tab)tab.close();alert('No se pudo generar el Excel. Revisa tu conexión e inténtalo nuevamente.')}finally{btn.disabled=false;btn.textContent=old}
  }
